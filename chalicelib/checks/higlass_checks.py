...
"""Patch the new HiGlass file to the given
"""

@check_function()
def check_for_bad_viewconf_static_content(connection, **kwargs):
    check = init_check_res(connection, 'check_for_bad_viewconf_static_content')
    check.full_output = {}
    check.action = 'remove_bad_viewconf_static_content'

    work_results = work_staic_content_higlass_viewconf(connection, False)

    check.full_output = work_results
    check.status = 'PASS'
    check.summary = check.description = "Ready to clear " + str(len(work_results["repaired"].keys())) + " files"
    return check

@action_function()
def remove_bad_viewconf_static_content(connection, **kwargs):
    action = init_action_res(connection, 'remove_bad_viewconf_static_content')
    gen_check = init_check_res(connection, 'check_for_bad_viewconf_static_content')
    ...
    work_results = work_staic_content_higlass_viewconf(connection, True)

    action.full_output = work_results
    action.status = 'DONE'
    action.output = work_results

    action.progress = "Cleared " + str(len(work_results["repaired"].keys())) + " files"
    return action

def work_staic_content_higlass_viewconf(connection, patch_static_content = False):
    repaired_files = {}
    too_much_static_content = {}

    # Query all processed files
    processed_file_query = '/search/?type=FileProcessed&frame=object'
    processed_file_res = ff_utils.search_metadata(processed_file_query, key=connection.ff_keys, ff_env=connection.ff_env)

    proc_file_count = 0
    content_file_count = 0

    for proc_file in processed_file_res:
        if proc_file_count >= 100:
            if proc_file_count % 25 == 0:
                print("{a}/{b} {c}".format(
                        a=proc_file_count,                b=len(processed_file_res),
                        c=content_file_count
                    )
                )
        proc_file_count += 1

        if "uuid" not in proc_file:
            continue

        # Get the static content, straight from the DB
        file_res = ff_utils.get_metadata(proc_file['uuid'], key=connection.ff_keys, ff_env=connection.ff_env, add_on="?frame=raw&datastore=database")

        file_static_content = file_res.get('static_content', [])

        if len(file_static_content) < 1:
            continue

        # Note if there's only 1 item
        exactly_one_item = len(file_static_content) == 1

        # Note if it has one of the purged view confs.
        higlass_static_content = [ sc for sc in file_static_content if sc['description'] == 'auto_generated_higlass_view_config' ]

        increment_counter = False
        # If it has static content with a higlass viewconf, try to access it
        for hsc in higlass_static_content:
            viewconf_uuid = hsc["content"]
            increment_counter = True

            if exactly_one_item:
                # Remove the file
                if patch_static_content:
                    response = ff_utils.delete_field(obj_id=proc_file['uuid'], del_field="static_content", key=connection.ff_keys, ff_env=connection.ff_env)
                repaired_files[proc_file["uuid"]] = viewconf_uuid
            else:
                too_much_static_content[proc_file["uuid"]] = viewconf_uuid

        # I'm gonna stop every 100 found files or so.
        if increment_counter:
            content_file_count += 1
            if content_file_count > 15:
                break

    return {
        "repaired" : repaired_files,
        "too_much" : too_much_static_content,
    }
