...
class TestCheckResult():
    ...
    def test_get_result_history(self, use_es):
        ...
        if check.connections['es'] is not None:
            check.connections['es'].refresh_index()
        hist_10, total_unused = check.get_result_history(0, 10)
        ...
        if len(hist_10) != 10:
            return
        hist_offset, total_unused = check.get_result_history(1, 2)
        hist_offset2, total_unused = check.get_result_history(2, 2)
        ...
        hist_after, total_unused = check.get_result_history(0, 10, after_date=after_date)
