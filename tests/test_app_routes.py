...
class TestAppRoutes():
    ...
    def test_view_foursight(self):
        res = self.app_utils_obj.view_foursight(self.environ) # not is_admin
        ...
        res2 = self.app_utils_obj.view_run_check(self.environ, 'indexing_progress', {})
        ...
        res3 = self.app_utils_obj.view_run_action(self.environ, 'add_random_test_nums', act_kwargs)
        assert (res3.status_code == 302)
        assert (res3.body == res2.body)
        # running action w/o check info gives 200 with action info
        res4 = self.app_utils_obj.view_run_action(self.environ, 'add_random_test_nums', {})
        assert (res4.status_code == 200)
        assert ('Action is queued.' in res4.body['details'])
        res = self.app_utils_obj.view_foursight(self.environ, True) # is_admin
        assert (res.status_code == 200)
    ...
    def test_view_foursight_check(self):
        ...
        res = self.app_utils_obj.view_foursight_check(self.environ, test_check_name, uuid)
        assert (res.status_code == 200)
        assert ('<!DOCTYPE html>' in res.body)
        assert ('Foursight' in res.body)
    ...
    def test_view_foursight_history(self):
        ...
        res = self.app_utils_obj.view_foursight_history(self.environ, 'indexing_progress') # not admin
        assert (res.headers == {u'Content-Type': u'text/html'})
        assert (res.status_code == 200)
        assert ('<!DOCTYPE html>' in res.body)
        assert ('Foursight' in res.body)
        ...
        res = self.app_utils_obj.view_foursight_history('not_an_environment', test_check)
        assert ('<td>' not in res.body)
        # run with bad check
        res = self.app_utils_obj.view_foursight_history(self.environ, 'not_a_check')
        assert ('<td>' not in res.body)
        # run with is_admin
        res = self.app_utils_obj.view_foursight_history(self.environ, test_check, is_admin=True) # not admin
        assert (res.status_code == 200)
        assert ('Currently logged in as admin' in res.body)
        # run with some limits/starts
        res = self.app_utils_obj.view_foursight_history(self.environ, test_check, start=4, limit=2)
        assert (res.status_code == 200)
    ...
    def test_get_foursight_history(self):
        test_check = 'test_random_nums'
        history = self.app_utils_obj.get_foursight_history(self.conn, test_check, 0, 3)
        ...
        history = self.app_utils_obj.get_foursight_history(self.conn, test_check, 1, 4)
        first_uuid_2 = history[0][2]['uuid']
        assert (first_uuid_1 != first_uuid_2)
        assert (second_uuid_1 == first_uuid_2)
        assert (len(history) == 4)
        # bad check
        bad_history = self.app_utils_obj.get_foursight_history(self.conn, 'not_a_real_check', 0, 3)
        assert (bad_history == [])

    def test_run_get_environment(self):
        environments = self.app_utils_obj.init_environments()
        env_resp = self.app_utils_obj.run_get_environment(self.environ)
        ...
        # bad environment
        resp2 = self.app_utils_obj.run_get_environment('not_an_environment')
    ...
    def test_put_environment(self):
        ...
        get_res = self.app_utils_obj.run_get_environment(self.environ)
        ...
        env_res = self.app_utils_obj.run_put_environment(self.environ, env_data)
        ...
        # failure case
        bad_res = self.app_utils_obj.run_put_environment(self.environ, {'key1': 'res1'})
        ...
        get_res2 = self.app_utils_obj.run_get_environment(self.environ)
        assert (get_res.body == get_res2.body)

    def test_run_get_check(self):
        test_check = 'indexing_progress'
        res = self.app_utils_obj.run_get_check(self.environ, test_check)
        assert (res.status_code == 200)
        ...
        # bad response
        res = self.app_utils_obj.run_get_check(self.environ, 'not_a_real_check')
    ...
    def test_put_check(self):
        ...
        res = self.app_utils_obj.run_put_check(self.environ, check_name, put_data)
        assert (res.status_code == 200)
